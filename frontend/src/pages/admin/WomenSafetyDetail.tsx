...
export const WomenSafetyDetail = () => {
  const { id } = useParams();

  const { data, isLoading } = useQuery({
    queryKey: ['women-safety', id],
    queryFn: async () => {
      const response = await fetch(`/api/women-safety/${id}`, {
    ...
    <Layout>
      <div className="page-content">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 600, color: '#fff' }}>Women Safety Record Details</h2>
          <Link to="/admin/women-safety">
          ...
          <div className="card" style={{ padding: '32px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '32px' }}>
              ...
              <div className="detail-section">
                <h3 style={{ fontSize: '1.1rem', color: '#a5b4fc', marginBottom: '16px', borderBottom: '1px solid rgba(165, 180, 252, 0.2)', paddingBottom: '8px' }}>Complaint Information</h3>
                ...
                <DetailRow label="Complaint Source" value={record.complaintSource} />
              ...
              <div className="detail-section">
                <h3 style={{ fontSize: '1.1rem', color: '#a5b4fc', marginBottom: '16px', borderBottom: '1px solid rgba(165, 180, 252, 0.2)', paddingBottom: '8px' }}>Incident Information</h3>
                <DetailRow label="Incident Type" value={record.incidentType} />
                <DetailRow label="Incident Place" value={record.incidentPlc} />
                <DetailRow label="From Date" value={record.incidentFromDt ? new Date(record.incidentFromDt).toLocaleDateString() : '-'} />
                <DetailRow label="To Date" value={record.incidentToDt ? new Date(record.incidentToDt).toLocaleDateString() : '-'} />
              </div>
...
export default WomenSafetyDetail;
